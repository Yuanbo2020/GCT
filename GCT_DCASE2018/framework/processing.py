...
def forward_greedy_search_validation(model, generate_func, cuda, generator, reverse,  verbose=0, ):
    ...
    for num, data in enumerate(generate_func):
        if len(data) == 5:
            batch_x, batch_y_seq, batch_y_seq_len, batch_y_weak, batch_audio_names = data
        else:
...
def training_process(generator, model, models_dir, epochs,
    ...
    # Optimizer
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=lr_init, momentum=0.99)

    train_val_data = pickle.load(open(config.training_val_file, 'rb'))
    train_audio_indexes = train_val_data['train_names']
    ...
    for iteration, all_data in enumerate(generator.generate_train()):
        ...
        batch_x, batch_y_sequence, batch_y_len, batch_y_sequence_reverse, batch_y_sequence_reverse_context = all_data
        sub_x = move_data_to_gpu(batch_x, cuda)
        sub_y = move_data_to_gpu(batch_y_sequence, cuda)
        sub_y_reverse = move_data_to_gpu(batch_y_sequence_reverse, cuda)
        sub_y_reverse_context = move_data_to_gpu(batch_y_sequence_reverse_context, cuda)

        model.train()
        optimizer.zero_grad()
        tgt_in = sub_y[:, :-1]
        tgt_y = sub_y[:, 1:]
        tgt_in_reverse = sub_y_reverse[:, :-1]
        tgt_y_reverse_context = sub_y_reverse_context[:, 1:]

        dec_logits, reverse_dec_logits = model(enc_inputs=sub_x, dec_inputs=tgt_in,
                                               reverse_dec_inputs=tgt_in_reverse,
                                               using_reverse=True, batch_y_len=batch_y_len)
        ...
        loss_re = criterion(reverse_dec_logits.contiguous().view(-1, generator.words_num),
                            tgt_y_reverse_context.contiguous().view(-1))
...
def initialization_model(model):
    ...
    best_model_dir = os.path.join(ast_dir, 'AST', 'pretrain_model')
    if config.cuda:
