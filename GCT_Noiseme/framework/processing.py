...
def model_predict(reverse, generator, model, each_x, each_seq_y):
    device = each_x.device
    if reverse:
        start_symbol_ind = generator.v2i[generator.end_string],
        end_symbol_ind = generator.v2i[generator.start_string]
    else:
    ...
    if reverse:
        ground_truth_each_seq_y = each_seq_y[::-1]
    else:
        ground_truth_each_seq_y = each_seq_y
    ground_truth_each_seq_y_only_event = sequence_real_event(generator, ground_truth_each_seq_y)


    return pred_y_only_event, ground_truth_each_seq_y_only_event
...
def forward_greedy_search_validation(model, generate_func, cuda, generator, reverse,
                                     verbose=0):
    ...
    for num, data in enumerate(generate_func):
        ...
        for j in range(len(batch_x)):
            ...
            pred_y_only_event, each_seq_y_only_event = model_predict(reverse, generator, model,
                                                                     each_x, each_seq_y)
...
def testing_greedy_search(model, generate_func, cuda, generator, reverse, verbose=0, start_end_labels=False):
    ...
    for _, data in enumerate(generate_func):
        ...
        for j in range(len(batch_x)):
            ...
            each_x = np.expand_dims(batch_x[j], axis=0)
            each_x = move_data_to_gpu(each_x, cuda)
            reverse_each_x = torch.flip(each_x, dims=[1])

            each_seq_y = batch_y_seq[j]

            pred_y_only_event, each_seq_y_only_event = model_predict_bidirection(generator, model, each_x,
                                                                                 reverse_each_x, each_seq_y,)
...
def training_process(generator, model, models_dir, epochs,
                               lr_init = 1e-3,
                               learning_rate_decay=False,
                                   cuda=config.cuda, reverse=False,):

    create_folder(models_dir)

    # Optimizer
    optimizer = optim.SGD(model.parameters(), lr=lr_init, momentum=0.99)
    ...
    decay_start_epoch = epochs/10 if epochs>999 else epochs/5  # 从第几轮开始优化
    ...
    for iteration, all_data in enumerate(generator.generate_train()):
        ...
        batch_x, batch_y_sequence, batch_y_len, \
        _, batch_y_sequence_reverse_input = all_data

        sub_x = move_data_to_gpu(batch_x, cuda)
        sub_y = move_data_to_gpu(batch_y_sequence, cuda)
        reverse_input_y = move_data_to_gpu(batch_y_sequence_reverse_input, cuda)

        model.train()
        optimizer.zero_grad()
        normal_branch_in = sub_y[:, :-1]
        same_target = sub_y[:, 1:]
        reverse_branch_in = reverse_input_y[:, :-1]

        dec_logits, reverse_dec_logits = model(enc_inputs=sub_x, dec_inputs=normal_branch_in,
                                               reverse_dec_inputs=reverse_branch_in,
                                               using_reverse=True,
                                               batch_y_len=batch_y_len)

        loss = criterion(dec_logits.contiguous().view(-1, generator.words_num),
                         same_target.contiguous().view(-1))

        loss_re = criterion(reverse_dec_logits.contiguous().view(-1, generator.words_num),
                            same_target.contiguous().view(-1))

        mse_loss = mse(reverse_dec_logits, dec_logits)

        final_loss = loss + loss_re + mse_loss
        ...
        if iteration % check_iter == 0 and iteration > 0:  # epoch end
...
def initialization_model(model):
    if os.path.exists("D:\Yuanbo\Dataset\DCASE2018_T1_A"):
        ast_dir = "D:\Yuanbo\Dataset\DCASE2018_T1_A"
    elif os.path.exists("E:\yuanbo\Dataset\DCASE2018_T1_A"):
        ast_dir = "E:\yuanbo\Dataset\DCASE2018_T1_A"
    elif os.path.exists("/project_antwerp/yuanbo/Dataset/DCASE2018_T1_A"):
        ast_dir = "/project_antwerp/yuanbo/Dataset/DCASE2018_T1_A"
    elif os.path.exists("/project_scratch/yuanbo/Dataset/DCASE2018_T1_A"):
        ast_dir = "/project_scratch/yuanbo/Dataset/DCASE2018_T1_A"
    best_model_dir = os.path.join(ast_dir, 'AST', 'best_model')
    if config.cuda:
        pretrained_model = torch.load(os.path.join(best_model_dir, 'audioset_10_10_0.4593.pth'),
    ...
    for k, v in model_dict.items():
        # print(k, v.size())
        if 'encoder.' in k:
            old_name = 'module.v.' + k.split('encoder.')[1]
            if old_name in pretrained_model.keys():
                state_dict[k] = pretrained_model[old_name]
...
def model_predict_bidirection(generator, model, each_x, reverse_each_x, each_seq_y, norm=True):
    device = each_x.device
    ...
    with torch.no_grad():
        ...
        reverse_enc_outputs, reverse_enc_self_attns = model.encoder(reverse_each_x)
        ...
        for i in range(generator.max_length - 1):
            ...
            if norm:
                ...
                dec_outputs = sep_dec_outputs * (1 - gated_dec_outputs) + dec_outputs * gated_dec_outputs
            projected = dec_outputs[:, -1]

            reverse_dec_outputs, _, _ = model.decoder(dec_inputs=reverse_dec_input,
            ...
            if norm:
                reverse_dec_logits = model.layerNorm(sep_reverse_dec_outputs * (1 - gated_reverse_dec_outputs) +
                                                    reverse_dec_outputs * gated_reverse_dec_outputs)
            else:
                reverse_dec_logits = sep_reverse_dec_outputs * (1 - gated_reverse_dec_outputs) + \
                                                     reverse_dec_outputs * gated_reverse_dec_outputs
            reverse_projected = reverse_dec_logits[:, -1]

            alpha = 0.5
            contextual_projected = projected * alpha + reverse_projected * alpha
