...
class Encoder_nopatch_simplec768t512(nn.Module):
    def __init__(self, n_layers, embed_dim):
        super(Encoder_nopatch_simplec768t512, self).__init__()
        ...
        self.convert768_512 = nn.Linear(embed_dim, config.d_model)

        self.c_128_to_768 = nn.Linear(128, embed_dim)

        self.init_weight()

    def init_weight(self):
        init_layer(self.c_128_to_768)
        init_layer(self.convert768_512)
    ...
    def forward(self, enc_inputs):
        enc_inputs = self.c_128_to_768(enc_inputs)
...
class Gated_cTransformer_clip(nn.Module):
    def __init__(self, ntoken, encoder_layers, decoder_layers, d_model=config.d_model, embed_dim = 768):
        super(Gated_cTransformer_clip, self).__init__()

        self.encoder = Encoder_nopatch_simplec768t512(n_layers=encoder_layers, embed_dim=embed_dim)
