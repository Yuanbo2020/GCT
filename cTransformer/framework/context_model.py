...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs):
        ...
        # print(enc_inputs.size())  # torch.Size([64, 1212, 768])
...
class Encoder_sat(nn.Module):
    def __init__(self, n_layers):
        super(Encoder_sat, self).__init__()
...
class Transformer_context(nn.Module):
    def __init__(self, ntoken, encoder_layers, decoder_layers, d_model=config.d_model):
        ...
        self.encoder = Encoder_sat(n_layers=encoder_layers)
    ...
    def forward(self, enc_inputs, dec_inputs, reverse_dec_inputs, using_reverse, batch_y_len):
        ...
        # print(dec_outputs.size())   # torch.Size([32, 16, 512])

        dec_logits = self.projection(dec_outputs)  # dec_logits : [batch_size x src_vocab_size x tgt_vocab_size]

        reverse_dec_logits = self.projection(reverse_dec_outputs)
