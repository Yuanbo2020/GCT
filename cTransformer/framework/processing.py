...
def model_predict_sat(reverse, generator, model, each_x, each_seq_y):
...
def forward_greedy_search_validation(model, generate_func, cuda, generator, reverse,
    ...
    for num, data in enumerate(generate_func):
        ...
        for j in range(len(batch_x)):
            ...
            pred_y_only_event, each_seq_y_only_event = model_predict_sat(reverse, generator, model,
                                                                     each_x, each_seq_y)
...
def training_process(generator, model, models_dir, epochs,
                               lr_init = 1e-3,
                               learning_rate_decay=False,   testing=False,
                                   cuda=config.cuda, reverse=False, start_end_labels=False):
